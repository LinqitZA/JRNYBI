...
# -----------------------------------------------------------------------------
# JRNYBI Feature #218: "Explain this number" LLM endpoint
# -----------------------------------------------------------------------------
# Anthropic API key. When unset, the /api/explain endpoint returns 503 (the
# feature is fully disabled). When set, the endpoint accepts metric + value +
# comparison context and returns a paragraph explanation generated by Claude.
ANTHROPIC_API_KEY = os.environ.get("ANTHROPIC_API_KEY", "")
EXPLAIN_FEATURE_ENABLED = parse_boolean(
    os.environ.get("REDASH_EXPLAIN_FEATURE_ENABLED", "true" if ANTHROPIC_API_KEY else "false")
)
# Anthropic model id. Defaults to the small/fast Sonnet line because the prompt
# is short and we want sub-second responses.
EXPLAIN_MODEL = os.environ.get("REDASH_EXPLAIN_MODEL", "claude-sonnet-4-5-20250929")
# Anthropic API endpoint (overridable for tests / proxies).
EXPLAIN_ANTHROPIC_URL = os.environ.get("REDASH_EXPLAIN_ANTHROPIC_URL", "https://api.anthropic.com/v1/messages")
EXPLAIN_ANTHROPIC_VERSION = os.environ.get("REDASH_EXPLAIN_ANTHROPIC_VERSION", "2023-06-01")
# Generation knobs.
EXPLAIN_MAX_TOKENS = int(os.environ.get("REDASH_EXPLAIN_MAX_TOKENS", "400"))
EXPLAIN_TIMEOUT_SECONDS = float(os.environ.get("REDASH_EXPLAIN_TIMEOUT_SECONDS", "20"))
# Cache TTL for (metric, time-range, value, comparison) tuples — repeat calls
# inside the window return the same cached explanation. Cost control.
EXPLAIN_CACHE_TTL_SECONDS = int(os.environ.get("REDASH_EXPLAIN_CACHE_TTL_SECONDS", "3600"))
# Per-user rate limit (sliding window via Redis INCR + EXPIRE).
EXPLAIN_RATE_LIMIT_PER_HOUR = int(os.environ.get("REDASH_EXPLAIN_RATE_LIMIT_PER_HOUR", "60"))
