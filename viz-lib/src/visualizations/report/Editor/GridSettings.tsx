...
export default function GridSettings({ options, onOptionsChange }: any) {
  return (
    // @ts-expect-error ts-migrate(2745) FIXME: This JSX tag's 'children' prop expects type 'never
    <Section>
      <Select
        label="Items per page"
        data-test="Report.ItemsPerPage"
        ...
        {map(ALLOWED_ITEM_PER_PAGE, (value) => (
          ...
          <Select.Option key={`ipp${value}`} value={value} data-test={`Report.ItemsPerPage.${value}`}>
            {value}
            {/* @ts-expect-error ts-migrate(2339) FIXME: Property 'Option' does not exist on type */}
          </Select.Option>
        ))}
